<?php
/**
 * Archive Template: 税理士事務所アーカイブ
 * 
 * 設計思想:
 * - 税理士サービスと税理士事務所を統合表示
 * - 検索・フィルタリング機能を実装
 * - 並び順はクエリ関数で制御
 * - ページネーション対応（/services/page/2/）
 * - 使いやすいUIデザイン
 * 
    ...
    <div class="container">
        ...
        <section class="service-section tax-office-section">
            ...
            <div class="service-archive">
                ...
                <?php
                // 税理士事務所のクエリ
                // アーカイブページなのget_query_var()でpagedを取得
                ...
                ?>
                    ...
                    <?php if ($offices_query->max_num_pages > 1): ?>
                        <div class="pagination">
                            <?php
                            // 検索パラメータを保持してページネーションを生成
                            $pagination_args = array(
                                'total' => $offices_query->max_num_pages,
                                'current' => $paged,
                                'format' => '?paged=%#%',
                                'add_args' => array(
                                    'office_industry' => isset($_GET['office_industry']) ? $_GET['office_industry'] : '',
                                    'office_service' => isset($_GET['office_service']) ? $_GET['office_service'] : '',
                                    'office_prefecture' => isset($_GET['office_prefecture']) ? $_GET['office_prefecture'] : '',
